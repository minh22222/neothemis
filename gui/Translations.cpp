...
const UiTextEntry kUiText[] = {
    ...
    {"server_already_running", "The local judging server is already running.", u8"Máy chủ chấm cục bộ đang chạy."},
    ...
    {"server_database_missing", "Server database has not been created yet.", u8"CSDL máy chủ chưa được tạo."},
    {"server_settings", "Server Settings", u8"Cài đặt máy chủ"},
    {"server_users", "Server Users", u8"Người dùng máy chủ"},
    {"server_settings_help", "Server accounts, submissions, and jobs are stored in the open contest folder under .neothemis-server. Passwords are visible to local GUI administrators and stored as plain text. Protect the contest folder.", u8"Tài khoản, bài nộp và tác vụ máy chủ được lưu trong thư mục kỳ thi đang mở dưới .neothemis-server. Mật khẩu hiển thị cho quản trị viên GUI cục bộ và được lưu dạng văn bản thường. Hãy bảo vệ thư mục kỳ thi."},
    ...
    {"server_enable_contestant_details", "Contestants can see own judge details", u8"Thí sinh được xem chi tiết bài nộp của mình"},
    {"server_user_username", "Username", u8"Tên đăng nhập"},
    {"server_user_password", "Password", u8"Mật khẩu"},
    {"server_user_role", "Role", u8"Vai trò"},
    {"server_username_hint", "letters, digits, spaces, underscore, dash", u8"chữ, số, khoảng trắng, gạch dưới, gạch ngang"},
    {"server_role_contestant", "Contestant", u8"Thí sinh"},
    {"server_role_admin", "Admin", u8"Quản trị"},
    {"server_role_invalid", "Server user role is invalid.", u8"Vai trò người dùng máy chủ không hợp lệ."},
    {"server_username_invalid", "Username must be 1-64 characters: letters, digits, spaces, underscore, dash.", u8"Tên đăng nhập phải dài 1-64 ký tự: chữ, số, khoảng trắng, gạch dưới, gạch ngang."},
    {"server_password_invalid", "Password must be between 6 and 128 characters.", u8"Mật khẩu phải dài từ 6 đến 128 ký tự."},
    {"server_user_exists", "A server user with that username already exists.", u8"Đã có người dùng máy chủ với tên đăng nhập này."},
    ...
    {"select_server_user", "Select a server user first.", u8"Hãy chọn một người dùng máy chủ trước."},
    ...
    {"remove_server_user_confirm", "Remove %1? Their server submissions and sessions will also be deleted. The contestant folder is kept.", u8"Xóa %1? Bài nộp và phiên đăng nhập trên máy chủ của họ cũng sẽ bị xóa. Thư mục thí sinh vẫn được giữ."},
    ...
    {"server_last_admin", "The final admin account cannot be removed.", u8"Không thể xóa tài khoản quản trị cuối cùng."},
    ...
    {"csv_users_filter", "CSV users (*.csv);;All files (*)", u8"Người dùng CSV (*.csv);;Tất cả tệp (*)"},
    {"server_users_csv_help", "CSV import columns: username,password,role. The role column is optional and defaults to contestant.", u8"Cột CSV: username,password,role. Cột role không bắt buộc và mặc định là thí sinh."},
    {"server_users_imported", "Imported %1 server users, skipped %2 rows.", u8"Đã nhập %1 người dùng máy chủ, bỏ qua %2 dòng."},
    {"server_users_import_failed", "Server user CSV import failed", u8"Nhập CSV người dùng máy chủ thất bại"},
    {"server_user_created", "Created server user %1.", u8"Đã tạo người dùng máy chủ %1."},
    {"server_user_failed", "Server user operation failed", u8"Thao tác người dùng máy chủ thất bại"},
    {"created_at", "Created", u8"Đã tạo"},
    {"server_security_warning", "LAN mode lets other devices submit C++ to this machine. Use a trusted network or an isolated VM/container.", u8"Chế độ LAN cho phép thiết bị khác gửi C++ vào máy này. Hãy dùng mạng tin cậy hoặc VM/container cô lập."},
    ...
    {"server_credentials_required", "Join code and admin password are required.", u8"Cần mã tham gia và mật khẩu quản trị."},
    {"server_started", "Server started", u8"Máy chủ đã khởi động"},
    {"server_lan_warning", "LAN server is exposed to the network. Use only on a trusted or isolated network.", u8"Máy chủ LAN đang mở ra mạng. Chỉ dùng trên mạng tin cậy hoặc cô lập."},
    ...
    {"server_users_synced", "Server users synced: %1 created, %2 already existed, %3 removed, %4 skipped. New accounts use password 123456.", u8"Đã đồng bộ người dùng máy chủ: tạo %1, đã tồn tại %2, xóa %3, bỏ qua %4. Tài khoản mới dùng mật khẩu 123456."},
    {"contestants_imported", "Imported %1 contestants, skipped %2.", u8"Đã nhập %1 thí sinh, bỏ qua %2."},
